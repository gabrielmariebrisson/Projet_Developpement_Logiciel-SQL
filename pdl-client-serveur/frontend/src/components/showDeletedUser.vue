...
async function restoreUser(idUser: number){
  await api.restoreThisUser(idUser);
...
function GoToModify(id: number) {
  // Aller à la page de modification de l'utilisateur
...
<template>
  <div v-if="users.length > 0">
    ...
    <center>
      <br>
      <button @click.prevent="GoToUser()" class="button button-light-red">Voir les utilisateurs non supprimer</button>
      <br>
      <table class="user-table">
        ...
        <tbody>
          <tr v-for="(user, index) in filteredUsers()" :key="index">  
            ...
            <td>
              <button @click.prevent="restoreUser(user.id)" class="button button-green">Restorer</button>
              <br>
